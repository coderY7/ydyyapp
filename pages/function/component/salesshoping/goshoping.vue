					...
					<view v-show="foldMoreShow">
						...
						<view class="view-flex" v-else-if="v.type=='多选下拉框'">
							...
							<!-- <u-radio-group v-model="v.value" placement="row">
								...
								<u-radio key="A" label="待审核" name="A"></u-radio>
		...
		methods: {
				...
				Condition(dataes).then((res) => {
					...
					if(res.error_code==0){
						...
						for(var i in this.queryData){
							...
							if(this.queryData[i].colname=="采购状态"){
			...
			newOrder(){
				...
				OrderNew(dataes).then((res) => {
					...
					if(res.error_code==0){
						uni.navigateTo({
							url: `/pages/function/component/goshoping/cgxd?djbh=${res.djbh}&state=add`
						});
					}else{
						this.$refs.uToast.show({
							type:"error",
