<template>
	<view class="box" :style="skin">
		...
		<view class="nav-bar">
			...
			<view class="center" v-show="ifpage">补货新单</view>
		...
		<view class="box-content" v-show="ifpage">
			...
			<view class="my-collapse">
				...
				<view class="my-collapse-con" v-show="myCollShow">
					...
					<u-form class="form-card" labelPosition="left" :model="uFormTitle">
						...
						<u-form-item label="补货部门" :labelWidth="76" prop="bhbm" @tap="QueryBm(false)">
							<u-input placeholder="请选择补货部门" disabled disabledColor="#fff" v-model="uFormTitle.bhbm">
							</u-input>
						</u-form-item>
						<u-form-item label="补货分店" :labelWidth="76" prop="bhfd" @tap="queryMore(false, 'FDINFO', 'bhfd')">
							<u-input placeholder="请选择补货分店" disabled disabledColor="#fff" v-model="uFormTitle.bhfd">
							</u-input>
						</u-form-item>
			...
			<view v-show="contentShow">
				<u-form class="form-card" labelPosition="left" :model="uFormModel" :rules="uFormRules" ref="uForm">
					...
					<u-form-item label="补货数量" :labelWidth="74" prop="bhsl" v-show="doingindex>=1">
						<u-input placeholder="请输入补货数量" type="number" v-model="uFormModel.bhsl" :focus="focusObj.numFocus">
						</u-input>
			...
			<movable-view class="movable-view" :x="x" :y="y" direction="all" :animation="false" :out-of-bounds="true" v-show="!isVoiceMode">
				<view class="iconimgs guideJS2" @tap="changeDh">
					...
					<uni-transition mode-class="fade" class="uni-transition" ref="uniTransition" :show="true">
						...
						<view class="my-fab-view" style="width:20px;"></view>
					</uni-transition>
				</view>
			</movable-view>
...
<script>
	...
	export default {
					...
					"spbm": {
						asyncValidator: (rule, value, callback) => {
							if(value.replace(/[^\x00-\xff]/g, "xx").length>=4){
								...
								callback(new Error("请输入4位及以上的商品编码/名称/条码查询"));
							}
						}
					},
					"bhsl": [{
		...
		onLoad(option) {
			...
			this.QueryBm(true)
			this.queryMore(true, "FDINFO", "bhfd")
		},
		onReady() {
			// 设置状态栏文字颜色为 白色
			// #ifdef APP-PLUS
		...
		methods: {
			...
			QueryBm(isauto){
				...
				Queryall(dataes).then((res) => {
					console.log("bbbbbbbbbbmmmmmmmmmm res",res)
					if(res.error_code==0){
						if (isauto) { //自动填充
							this.uFormTitle.bhbm = `${res.nodes[0].id}-${res.nodes[0].title.split(".")[1]}`
							...
							for(var i in res.nodes){
								this.selectData.push({
									"id": res.nodes[i].id,
									"name": res.nodes[i].title.split(".")[1]
								})
							}
						}
					}else{
					...
					success: (resm)=> {
						if (resm.confirm) {
							let dataes = {
								...
								"fdbh": this.uFormTitle.bhfd.split("-")[0],
								"remark": this.uFormTitle.remarks
							}
							BhdCheck(dataes).then((res) => {
			...
			searchSpckl(spbm){
				...
				BhdSpckl(dataes).then((res) => {
					...
					if (res.error_code == 0) {
						this.uFormModel.yjxl=res.data[0].yjxl
						this.uFormModel.ybhl=res.data[0].ybhl
						this.uFormModel.cxzt=res.data[0].cxzt
						this.uFormModel.zbkcl=res.data[0].zbkcl
						this.uFormModel.psckl=res.data[0].psckl
					} else {
			...
			save() {
				this.$refs.uForm.validate().then(resf => {
					this.uploadarr = []
					this.uploadarr.push({
						"ybhl": this.uFormModel.yjxl,
						"bhsl": this.uFormModel.bhsl,
						"yjxl": this.uFormModel.yjxl,
						"spbm": this.uFormModel.spbm,
						"spsmm": this.uFormModel.spsmm,
						"spkcl": this.uFormModel.psckl,
						"zbkcl": this.uFormModel.zbkcl,
			...
			doSave(state) {
				let dataes = {
					...
					"vtype": state,
					"djbh": this.uFormTitle.djbh,
					"bhfd": this.uFormTitle.bhfd.split("-")[0],
					"fdbh": uni.getStorageSync("fdbh"),
					"depart": this.uFormTitle.bhbm.split("-")[0],
					"remark": this.uFormTitle.remarks,
					"list": this.uploadarr
				}
				console.log("state=="+state+"; 保存商品 dosave dataes", dataes)
				BhdDosave(dataes).then((res) => {
