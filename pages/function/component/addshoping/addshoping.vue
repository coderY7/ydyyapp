				...
				<view class="" v-for="(v,i) in queryData">
					...
					<view v-show="foldMoreShow">
						<view class="view-flex" v-if="v.type=='查询下拉框'">
						 ...
						 <!-- @tap="queryMore(v,i,v.type,'ALL')" -->
						<view class="view-flex" v-else-if="v.type=='下拉框'">
							<text class="form-left-text">{{v.colname}}</text>
							<u-radio-group v-model="v.value" placement="column" v-if="v.codeid=='SK'">
		...
		methods: {
			...
			queryMore(obj,index,state,isAll){
				...
				Basic(dataes).then((res) => {
					...
					if(obj.readonly!=""){//补货分店
						for(var i in res.data){
							if(uni.getStorageSync("fdbh")==res.data[i][obj.tabname.id]){
								this.$set(this.queryData[this.selectIndex], "value", `${res.data[i][obj.tabname.id]}-${res.data[i][obj.tabname.name]}`)
							}
						}
						this.$forceUpdate()
						return
			...
			newOrder(){
				...
				OrderNew(dataes).then((res) => {
					...
					if(res.error_code==0){
						uni.navigateTo({
							url: `/pages/function/component/addshoping/bhxd?djbh=${res.djbh}&state=add`
						});
					}else{
						this.$refs.uToast.show({
							type:"error",
			...
			tolook(item){
				...
				uni.navigateTo({
					url: `/pages/function/component/addshoping/bhxd?djbh=${item.补货单号}&state=${states}&bhfd=${item.补货分店}&bhzt=${item.采购状态}&bhzl=${item.补货总量}&bhje=${item.补货金额}&spzl=${item.实配总量}&djzt=${item.单据状态}`
				});
			},
