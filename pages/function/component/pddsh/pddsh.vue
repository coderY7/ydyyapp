<template>
	<view class="box" :style="skin">
		...
		<view class="nav-bar">
			...
			<view class="center">配调单收货处理</view>
		...
		<view class="box-content">
			...
			<u-form ref="uForm" :model="uFormModel">
				...
				<u-form-item label="配调单号:" :labelWidth="90" prop="pddh" borderBottom>
					<u-input placeholder="配调单号" v-model="uFormModel.pddh" border="none">
					</u-input>
				</u-form-item>
				...
				<u-form-item label="分店编号:" :labelWidth="90" prop="fdbh" borderBottom>
					<u-input v-model="uFormModel.fdbh" disabled border="none">
					</u-input>
				</u-form-item>
	...
	export default {
		...
		methods: {
			...
			search(){
				...
				let pddh = this.uFormModel.pddh
				let spsmm = this.uFormModel.spsmm
				let fendian = uni.getStorageSync("fdbh")
				let exeStr = "'PHQRD','" + pddh + "','" + start + "','" + end + "','" + '' + "','T','" + spsmm + "','" + fendian + "'"
				let datas = JSON.stringify('GETORDERLISTINFO${"fdbh":"' + fendian + '","exeStr":"' + exeStr + '","checkbox":' + 'false' + ',"showsum":false}')
				console.log("配调单收货 datas",datas)
				commonMain(datas).then((res) => {
					console.log("配调单收货 res",res)
					if(res.retflag==0){
						this.thData=[]
