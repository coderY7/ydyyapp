					...
					<view v-show="foldMoreShow">
						...
						<!-- <view class="view-flex" v-else-if="v.type=='多选下拉框'">
							...
							<u-radio-group v-model="v.value" placement="row">
								...
								<u-radio key="ALL" label="全部" name="ALL"></u-radio>
							</u-radio-group>
						</view> -->
						<view class="view-flex" v-else-if="v.type=='多选下拉框'">
							<text class="form-left-text">{{v.colname}}</text>
			...
			<view class="foldGroup">
				<view class="fold-title" v-for="(v,i) in tableData" @tap="tolook(v)">
					...
					<view class="fold-title-flex-start fold-title-con show-dots">
						<text class="left-con">过帐单号:</text>
						<text class="right-con">{{v.过帐单号}}</text>
					</view>
					<view class="multiples">
						...
						<view class="multiple-con">
							<text class="left-con">仓库分类:</text>
							<text class="right-con">{{v.ckbmid}}</text>
						</view>
					</view>
					<view class="multiples">
						<view class="multiple-con">
							<text class="left-con">盘点总数:</text>
							<text class="right-con">{{v.盘点总数}}</text>
						</view>
						<view class="multiple-con">
							<text class="left-con">损溢总数:</text>
							<text class="right-con">{{v.损溢总数}}</text>
						</view>
					</view>
					<view class="multiples">
						<view class="multiple-con">
							<text class="left-con">成本总额:</text>
							<text class="right-con">{{v.盘点成本总额}}</text>
						</view>
						<view class="multiple-con">
							<text class="left-con">零售总额:</text>
							<text class="right-con">{{v.盘点零售总额}}</text>
						</view>
					</view>
					<view class="multiples">
						<view class="multiple-con">
							<text class="left-con">损溢总额:</text>
							<text class="right-con">{{v.损溢成本总额}}</text>
						</view>
						<view class="multiple-con">
		...
		methods: {
			...
			newOrder(){
				...
				OrderNew(dataes).then((res) => {
					...
					if(res.error_code==0){
						uni.navigateTo({
							url: `/pages/function/component/checkshoping/pdxd?djbh=${res.djbh}&state=add`
						});
					}else{
						this.$refs.uToast.show({
							type:"error",
			...
			tolook(item){
				...
				uni.navigateTo({
					url: `/pages/function/component/checkshoping/pdxd?djbh=${item.盘点单号}&pdck=${item.ckbmid}&state=${states}&fdbh=${item.分店编号}&pzs=${item.品种数}&pdzs=${item.盘点总数}&syzs=${item.损溢总数}&pdcbze=${item.盘点成本总额}&pdnsze=${item.盘点零售总额}&sycbze=${item.损溢成本总额}&gzdh=${item.过帐单号}&djzt=${item.单据状态}`
				});
			},
